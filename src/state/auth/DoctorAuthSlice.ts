...
export const { actionSuccess, actionFailed, setUser, setLoading, setMessage, clearMessageAndError, logout } = doctorAuth.actions;
  ...
  try {
    ...
    if (user !== null) {
      await updateProfile(user, {
        ...
        cookies.set('user', JSON.stringify(state.doctorAuth.user), { path: '/' });
      }).catch((error) => {
        dispatch(setLoading(false));
  ...
  if (getState().doctorAuth.user) {
    ...
    await signInWithEmailAndPassword(auth, email, password)
      .then(() => {
        ...
        if (getState().doctorAuth.error) {
          throw new Error("Failed to get user profile");
        }
        cookies.set('user', JSON.stringify(getState().doctorAuth.user), { path: '/' });
        dispatch(actionSuccess());
        dispatch(setMessage("User logged in successfully!"));
        console.log("User logged in successfully!", getState().doctorAuth.user);
      })
      .catch((error: any) => {
        dispatch(setLoading(false));
        dispatch(actionFailed({ code: error.code, message: error.message }));
