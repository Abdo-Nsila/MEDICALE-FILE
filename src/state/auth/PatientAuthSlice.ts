...
export const { actionSuccess, actionFailed, setUser, setLoading, setMessage, clearMessageAndError, logout } = patientAuth.actions;

export default patientAuth.reducer;
  ...
  try {
    ...
    if (user !== null) {
      await updateProfile(user, {
        ...
        cookies.set('user', JSON.stringify(state.patientAuth.user), { path: '/' });
      }).catch((error) => {
        dispatch(setLoading(false));
  ...
  if (getState().patientAuth.user) {
    ...
    await signInWithEmailAndPassword(auth, email, password)
      .then(() => {
        ...
        if (getState().patientAuth.error) {
          throw new Error("Failed to get user profile");
        }
        cookies.set('user', JSON.stringify(getState().patientAuth.user), { path: '/' });
        dispatch(actionSuccess());
        dispatch(setMessage("User logged in successfully!"));
        console.log("User logged in successfully!", getState().patientAuth.user);
      })
      .catch((error: any) => {
        dispatch(setLoading(false));
        dispatch(actionFailed({ code: error.code, message: error.message }));
